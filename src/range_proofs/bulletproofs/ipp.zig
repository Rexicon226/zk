...
/// elements ($L_i$ and $R_i$) that allow the verifier to reconstruct the
...
pub fn Proof(comptime bit_size: u64) type {
    ...
    return struct {
        ...
        pub fn init(
            ...
            var L_vec: stdx.BoundedArray(Ristretto255, logn) = .{};
            var R_vec: stdx.BoundedArray(Ristretto255, logn) = .{};
            ...
            inline for (0..rounds) |i| {
                ...
                var scalars: stdx.BoundedArray([32]u8, bit_size + 1) = .{};
                var points: stdx.BoundedArray(Ristretto255, bit_size + 1) = .{};
                ...
                const L = ed25519.mulMultiRuntime(
                    (n * 2) + 1,
                    false,
                    true,
                    points.constSlice(),
                ...
                const R = ed25519.mulMultiRuntime(
                    (n * 2) + 1,
                    false,
                    true,
                    points.constSlice(),
        ...
        pub fn verify(
            ...
            var scalars: stdx.BoundedArray([32]u8, max_elements) = .{};
            var points: stdx.BoundedArray(Ristretto255, max_elements) = .{};
            ...
            points.appendSliceAssumeCapacity(&self.L_vec);
            points.appendSliceAssumeCapacity(&self.R_vec);
...
test "basic correctness" {
    inline for (.{ 8, 16, 32, 64, 128, 256 }) |n| {
        errdefer std.log.err("failed on: {d}", .{n});
        ...
        // where b' = b ∘ y^(-n)
        const P_len = 2 * n + 1;
        var scalars: stdx.BoundedArray([32]u8, P_len) = .{};
        ...
        var points: stdx.BoundedArray(Ristretto255, P_len) = .{};
