...
pub const Strobe128 = Strobe(.{ .b = 1600, .security = 128 });

/// Strobe: https://strobe.sourceforge.io/specs/
pub fn Strobe(params: struct {
    /// May be either 400, 800, 1600.
    b: u32,
    /// The target security level, either 128 or 256 bits.
    security: u32,
}) type {
    return struct {
        state: Keccak,
        ...
        comptime {
            ...
            switch (b) {
                400, 800, 1600 => {},
                else => @compileError("invalid value for b"),
            }
            // Let sec be a target security level, either 128 or 256 bits.
            switch (sec) {
                128, 256 => {},
                else => @compileError("invalid value of security"),
            }

            // It is required that 1 ≤ R < 254.
            std.debug.assert(1 <= R and R < 254);
        }

        const Self = @This();
        const Keccak = std.crypto.core.keccak.KeccakF(b);
        ...
        pub fn beginOp(self: *Self, flags: Flags, more: bool) void {
        ...
        fn permuteState(self: *Self) void {
        ...
        fn absorb(self: *Self, data: []const u8) void {
            const state: *[200]u8 = @ptrCast(&self.state.st);
            for (data) |byte| {
        ...
        fn squeeze(self: *Self, destination: []u8) void {
            const state: *[200]u8 = @ptrCast(&self.state.st);
        ...
        fn overwrite(self: *Self, destination: []const u8) void {
            const state: *[200]u8 = @ptrCast(&self.state.st);
            for (destination) |byte| {
                state[self.position] = byte;
                self.position += 1;
        ...
        pub fn metaAd(self: *Self, data: []const u8, more: bool) void {
            self.beginOp(.{ .M = true, .A = true }, more);
            self.absorb(data);
        }

        pub fn ad(self: *Self, data: []const u8, more: bool) void {
            self.beginOp(.{ .A = true }, more);
            self.absorb(data);
        }

        pub fn prf(self: *Self, destination: []u8, more: bool) void {
            self.beginOp(.{ .I = true, .A = true, .C = true }, more);
            self.squeeze(destination);
        }

        pub fn key(self: *Self, destination: []u8, more: bool) void {
