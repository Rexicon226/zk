...
pub fn mulMultiRuntime(
    ...
    var scalars_in_radix: stdx.BoundedArray([64]i8, max) = .{};
    for (scalars) |scalar| {
        scalars_in_radix.appendAssumeCapacity(asRadix16(scalar));
    }

    var lookup_tables: stdx.BoundedArray(LookupTable, max) = .{};
    for (points) |point| {
...
pub fn mulMulti(
    ...
    points: *const [N]Ristretto255,
    scalars: *const [N]CompressedScalar,
) Ristretto255 {
