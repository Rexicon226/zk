...
pub fn main() !void {
    ...
    log.info("using: ed25519 {s} backend", .{switch (zk.curves.ed25519.use_avx512) {
        true => "avx512",
    ...
    noinline fn deserialize() !void {
        {
            ...
            log.info("Falcon512: Pubkey.deserialize {D}", .{elapsed / iterations});
        }
        {
            const elapsed = bench(iterations, warmup, Falcon512.Signature.fromBytes, .{signature_bytes[41..]});
            log.info("Falcon512: Signature.deserialize {D}", .{elapsed / iterations});
        }
    }

    noinline fn hashToPoint() !void {
        const elapsed = bench(iterations, warmup, Falcon512.hashToPoint, .{ data, signature_bytes[1..][0..40] });
        log.info("Falcon512: hashToPoint {D}", .{elapsed / iterations});
    }
